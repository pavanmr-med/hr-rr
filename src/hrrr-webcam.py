...
class ICABasedHRVCalculator:
    ...
    def __init__(self):
        ...
        self.detected_peaks = deque(maxlen=60)
    ...
    def pos_method(self, rgb_signal):
        ...
        try:
            ...
            Xf = normalized[0] - normalized[2]
            Yf = 2 * normalized[1] - normalized[0] - normalized[2]
...
class AdvancedVitalSignsMonitor:
    def __init__(self):
        print("\n[*] Initializing Advanced Vital Signs Monitor for Laptop Camera...")
        ...
        try:
            ...
            # Get actual camera resolution and set appropriate size
            self.cap.set(cv2.CAP_PROP_FRAME_WIDTH, 640)
            self.cap.set(cv2.CAP_PROP_FRAME_HEIGHT, 480)
            print("✓ Laptop camera initialized (640x480)")
    ...
    def ensure_cascade_classifier(self):
        ...
        try:
            ...
            # Try to find cascade in OpenCV install
            import cv2
    ...
    def estimate_respiratory_rate(self, motion_signal):
        ...
        try:
            ...
            if motion_std < 0.1 or motion_mean < 0.05:
                return 0
    ...
    def process_frame(self, frame):
        ...
        if self.face_detected and len(self.motion_buffer) >= 50:
    ...
    def draw_peak_detection_graph(self, waveform_data, peaks, height=480, width=320):
        """Draw peak detection waveform graph - FIXED DIMENSIONS"""
    ...
    def run(self, duration=60):
        ...
        try:
            while (time.time() - start_time) < duration:
                ...
                peak_graph = self.draw_peak_detection_graph(
                    list(self.ppg_waveform_buffer), 
                    list(self.peak_positions), 
                    height=480,  # Match camera frame height
                    width=320
                )
                ...
                try:
                    ...
                    cv2.imshow('Vital Signs Monitor - ICA + Peak Detection', display_frame)
            ...
            print("\n--- SESSION STATISTICS ---")
            print(f"Valid Face Detection Frames:         {self.valid_face_frames}")
            print(f"Valid Motion Detection Frames:       {self.valid_motion_frames}")
            print("="*80)


def main():
    print("="*80)
    print("Advanced Vital Signs Monitor - ICA + Peak Detection (Laptop Camera)")
    print("Optimized for Laptop Webcam | 60 Seconds")
    print("="*80)
