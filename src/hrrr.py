...
- FIX: Only calculate RR when face is detected and motion is valid
...
class AdvancedVitalSignsMonitor:
    def __init__(self):
        ...
        # VALIDATION FLAGS
        self.valid_face_frames = 0  # Count frames with valid face detection
        self.valid_motion_frames = 0  # Count frames with valid motion data
    ...
    def estimate_respiratory_rate(self, motion_signal):
        ...
        try:
            ...
            # VALIDATION: Check if motion signal has sufficient variance
            motion_std = np.std(motion_array)
            motion_mean = np.mean(motion_array)
            
            # If motion is too small or all zeros, return 0
    ...
    def process_frame(self, frame):
        ...
        # FIXED: Only calculate RR if face is detected AND motion is valid
        if self.face_detected and len(self.motion_buffer) >= 50:
            ...
            # No face detected or insufficient motion buffer
            self.raw_rr = 0
            self.current_rr = 0
    ...
    def draw_peak_detection_graph(self, waveform_data, peaks, height=240, width=220):
...
def main():
    print("="*80)
    print("Advanced Vital Signs Monitor - ICA + Peak Detection (FIXED RR)")
